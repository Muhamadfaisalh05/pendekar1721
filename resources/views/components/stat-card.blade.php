@props(['icon', 'value', 'label', 'variant' => 'blue'])
...
<div class="stat-card {{ $variantClass }}">
    <div class="flex items-center gap-4">
        <div class="flex-shrink-0 w-14 h-14 rounded-xl {{ $iconBg }} flex items-center justify-center">
            @if($icon === 'users')
        ...
        <div>
            <p class="text-3xl font-bold {{ $valueColor }} tracking-tight">{{ $value }}</p>
            <p class="text-sm text-gray-500 mt-0.5 font-medium">{{ $label }}</p>
        </div>
    </div>
</div>
